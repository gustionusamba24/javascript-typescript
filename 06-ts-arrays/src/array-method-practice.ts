...
const bankDepositSum = accountsAP
  ...
  .reduce((sum: number, mov: number): number => sum + mov, 0);
...
const numDeposits1000 = accountsAP
  .flatMap((acc: AccountAP): number[] => acc.movements)
  .reduce(
    (count: number, cur: number): number => (cur >= 1000 ? count + 1 : count),
    0
  );
...
const { depositsAP, withdrawalsAP } = accountsAP
  ...
  .reduce(
    (
      sums: { depositsAP: number; withdrawalsAP: number },
      cur: number
    ): { depositsAP: number; withdrawalsAP: number } => {
...
const convertTitleCase = function (title: string) {
  ...
  const titleCase = title
    ...
    .split(" ")
    .map((word: string) => {
      if (exeptions.includes(word)) return word;
